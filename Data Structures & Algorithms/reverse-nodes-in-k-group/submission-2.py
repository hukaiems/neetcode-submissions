...
class Solution:
    ...
    def reverseList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        "use recursive"
        # so we need an if condition to find the last node 
        new_head = head
        if head.next:
            # call recursive if next node exist
            new_head = self.reverseList(head.next)
            # if return  then keep the new_head
            # use the method .next.next to connect to itself
            head.next.next = head
        # connect the current node to None for safety
        head.next = None
        return new_head
